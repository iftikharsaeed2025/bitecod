import { Fragment, Transition, computed, createBaseVNode, createBlock, createCommentVNode, createElementBlock, createTextVNode, createVNode, defineComponent, guardReactiveProps, inject, isRef, mergeProps, nextTick, normalizeClass, normalizeProps, normalizeStyle, onMounted, onUnmounted, openBlock, provide, reactive, ref, renderList, renderSlot, resolveComponent, resolveDirective, resolveDynamicComponent, toDisplayString, toHandlers, toRef, toRefs, unref, watch, watchEffect, withCtx, withDirectives, withKeys, withModifiers } from "./runtime-dom.esm-bundler-bxINyfmg.js";
import "./vue.runtime.esm-bundler-1YWvcuWz.js";
...
function effect$2(_ref2) {
	...
	return function() {
		Object.keys(state$1.elements).forEach(function(name) {
			var element = state$1.elements[name];
			var attributes = state$1.attributes[name] || {};
			var style = Object.keys(state$1.styles.hasOwnProperty(name) ? state$1.styles[name] : initialStyles[name]).reduce(function(style$1, property$2) {
				style$1[property$2] = "";
				return style$1;
			}, {});
			if (!isHTMLElement(element) || !getNodeName(element)) return;
...
function expandToHashMap(value, keys$4) {
	return keys$4.reduce(function(hashMap, key) {
		hashMap[key] = value;
		return hashMap;
	}, {});
}
...
function detectOverflow(state$1, options) {
	...
	var clippingClientRect = getClippingRect(isElement(element) ? element : element.contextElement || getDocumentElement(state$1.elements.popper), boundary, rootBoundary, strategy);
...
function offset(_ref2) {
	...
	var _data$state$placement = data[state$1.placement], x = _data$state$placement.x, y = _data$state$placement.y;
	if (state$1.modifiersData.popperOffsets != null) {
		state$1.modifiersData.popperOffsets.x += x;
		state$1.modifiersData.popperOffsets.y += y;
	}
	...
	return function createPopper$1(reference$1, popper$1, options) {
		...
		var instance = {
			...
			setOptions: function setOptions(setOptionsAction) {
				...
				state$1.options = Object.assign({}, defaultOptions$1, state$1.options, options$1);
				...
				state$1.orderedModifiers = orderModifiers(mergeByName([].concat(defaultModifiers$1, state$1.options.modifiers))).filter(function(m) {
					return m.enabled;
				});
				runModifierEffects();
...
function arrayPush$1(array, values) {
	var index2 = -1, length = values.length, offset$1 = array.length;
	while (++index2 < length) array[offset$1 + index2] = values[index2];
	return array;
}
...
function overRest$1(func, start$1, transform) {
	start$1 = nativeMax(start$1 === void 0 ? func.length - 1 : start$1, 0);
	return function() {
		var args = arguments, index2 = -1, length = nativeMax(args.length - start$1, 0), array = Array(length);
		while (++index2 < length) array[index2] = args[start$1 + index2];
		index2 = -1;
		var otherArgs = Array(start$1 + 1);
		while (++index2 < start$1) otherArgs[index2] = args[index2];
		otherArgs[start$1] = transform(array);
...
function baseRest$3(func, start$1) {
...
function clamp(num, min$1, max$1) {
...
function tzParseTimezone(timezoneString, date, isUtcDate) {
	...
	if (isValidTimezoneIANAString(timezoneString)) {
		...
		var offset$1 = calcOffset(utcDate, timezoneString);
		return -(isUtcDate ? offset$1 : fixOffset(date, offset$1, timezoneString));
	}
	return NaN;
}
...
function fixOffset(date, offset$1, timezoneString) {
	var utcGuess = date.getTime() - offset$1;
	var o2 = calcOffset(new Date(utcGuess), timezoneString);
	if (offset$1 === o2) return offset$1;
	utcGuess -= o2 - offset$1;
...
function toDate$1(argument, dirtyOptions) {
	...
	if (date) {
		...
		if (dateStrings.timeZone || options.timeZone) {
			offset$1 = tzParseTimezone(dateStrings.timeZone || options.timeZone, new Date(timestamp + time));
			if (isNaN(offset$1)) return /* @__PURE__ */ new Date(NaN);
		} else {
			offset$1 = getTimezoneOffsetInMilliseconds$1(new Date(timestamp + time));
			offset$1 = getTimezoneOffsetInMilliseconds$1(new Date(timestamp + time + offset$1));
		}
		return new Date(timestamp + time + offset$1);
...
var DateRange = class DateRange {
	constructor(config, locale = new Locale()) {
		...
		var _a;
		this.locale = locale;
		const { start: start$1, end: end$1, span, order: order$1, repeat } = config;
		if (isDate(start$1)) this.start = locale.getDateParts(start$1);
		if (isDate(end$1)) this.end = locale.getDateParts(end$1);
		else if (this.start != null && span) this.end = locale.getDateParts(addDays(this.start.date, span - 1));
		this.order = order$1 ?? 0;
	...
	get opts() {
		const { order: order$1, locale } = this;
		return {
			order: order$1,
			locale
		};
	...
	get isSingleDay() {
		const { start: start$1, end: end$1 } = this;
		return start$1 && end$1 && start$1.year === end$1.year && start$1.month === end$1.month && start$1.day === end$1.day;
	}
	get isMultiDay() {
...
function getDateFromParts(parts, timezone = "") {
	...
	const { year = d.getFullYear(), month = d.getMonth() + 1, day = d.getDate(), hours: hrs = 0, minutes: min$1 = 0, seconds: sec = 0, milliseconds: ms = 0 } = parts;
	if (timezone) {
		const dateString = `${pad(year, 4)}-${pad(month, 2)}-${pad(day, 2)}T${pad(hrs, 2)}:${pad(min$1, 2)}:${pad(sec, 2)}.${pad(ms, 3)}`;
		return toDate$1(dateString, { timeZone: timezone });
	}
	return new Date(year, month - 1, day, hrs, min$1, sec, ms);
}
...
function getDatePartOptions(parts, range, rule) {
	const options = [];
	const [min$1, max$1, padding] = range;
	for (let i = min$1; i <= max$1; i++) if (rule == null || datePartIsValid(i, rule, parts)) options.push({
		value: i,
		label: pad(i, padding)
	});
	return options;
...
var Attribute = class {
	...
	intersectsRange({ start: start$1, end: end$1 }) {
		if (start$1 == null || end$1 == null) return false;
		const simpleRanges = this.ranges.filter((r) => !r.hasRepeat);
		for (const range of simpleRanges) if (range.intersectsDayRange(start$1.dayIndex, end$1.dayIndex)) return true;
		...
		while (day.dayIndex <= end$1.dayIndex) {
...
var HighlightRenderer = class {
	...
	render({ data: data2, onStart, onEnd }, glyphs) {
		...
		const { base, start: start$1, end: end$1 } = highlight;
		if (onStart && onEnd) highlights.push({
			...start$1,
			key,
			wrapperClass: `vc-day-layer vc-day-box-center-center vc-attr vc-${start$1.color}`,
			class: [`vc-highlight vc-highlight-bg-${start$1.fillMode}`, start$1.class],
			contentClass: [`vc-attr vc-highlight-content-${start$1.fillMode} vc-${start$1.color}`, start$1.contentClass]
		});
		else if (onStart) {
			highlights.push({
				...base,
			...
			highlights.push({
				...end$1,
				key,
				wrapperClass: `vc-day-layer vc-day-box-center-center vc-attr vc-${end$1.color}`,
				class: [`vc-highlight vc-highlight-bg-${end$1.fillMode}`, end$1.class],
				contentClass: [`vc-attr vc-highlight-content-${end$1.fillMode} vc-${end$1.color}`, end$1.contentClass]
			});
...
var BaseRenderer = class {
	...
	render({ data: data2, onStart, onEnd }, glyphs) {
		...
		const { base, start: start$1, end: end$1 } = item;
		if (onStart) collection.push({
			...start$1,
			key,
			class: [`vc-${this.type} vc-${this.type}-start vc-${start$1.color} vc-attr`, start$1.class]
		});
		else if (onEnd) collection.push({
			...end$1,
			key,
			class: [`vc-${this.type} vc-${this.type}-end vc-${end$1.color} vc-attr`, end$1.class]
		});
	...
	setup(props, { emit }) {
		...
		function setupPopper() {
			nextTick(() => {
				...
				if (popper$1 && popper$1.state.elements.reference !== el) destroyPopper();
				if (!popper$1) popper$1 = createPopper(el, popoverRef.value, popperOptions.value);
				else popper$1.update();
			});
		}
		...
		function isCurrentTarget(target) {
			if (!target || !popper$1) return false;
			return resolveEl(target) === popper$1.state.elements.reference;
		}
		...
		function toggle(opts = {}) {
			...
			if (state2.isVisible && isCurrentTarget(opts.target)) hide$1(opts);
			else show(opts);
		}
		function onDocumentClick(e) {
			if (!popper$1) return;
			const popperRef = popper$1.state.elements.reference;
		...
		function onDocumentKeydown(e) {
			if (e.key === "Esc" || e.key === "Escape") hide$1();
		}
		...
		function onMouseLeave() {
			...
			if (!popper$1) return;
			const popperRef = popper$1.state.elements.reference;
			if (state2.autoHide && !state2.isFocused && (!popperRef || popperRef !== document.activeElement) && ["hover", "hover-focus"].includes(state2.visibility)) hide$1();
		}
		function onFocusIn() {
		...
		function onFocusOut(e) {
			if (["focus", "hover-focus"].includes(state2.visibility) && (!e.relatedTarget || !elementContains(popoverRef.value, e.relatedTarget))) {
				...
				if (!state2.isHovered && state2.autoHide) hide$1();
			}
		}
		function cleanupRO() {
	...
	setup(__props) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createBlock(Popover, {
				...
				default: withCtx(({ data: { day, attributes }, hide: hide$1 }) => [createVNode(_sfc_main$i, {
					name: "day-popover",
					day,
...
var IconClock = /* @__PURE__ */ _export_sfc(_sfc_main$d, [["render", _sfc_render$3]]);
...
function createDatePicker(props, { emit, slots }) {
	...
	function normalizeValue(value, config2, patch, targetPriority) {
		...
		if (hasRangeValue(value)) {
			const start$1 = locale.value.toDate(value.start, {
				...config2[0],
				fillDate: valueStart.value ?? void 0,
				patch
			});
			const end$1 = locale.value.toDate(value.end, {
				...config2[1],
	...
	function updateValue(value, opts = {}) {
		...
		return new Promise((resolve) => {
			const { debounce: debounce$1 = 0,...args } = opts;
			if (debounce$1 > 0) updateTimeout = window.setTimeout(() => {
	...
	function sortRange(range, priority) {
		const { start: start$1, end: end$1 } = range;
		if (start$1 > end$1) switch (priority) {
			case "start": return {
				start: start$1,
				end: start$1
			};
	...
	async function moveToValue(target, opts = {}) {
		...
		const start$1 = target !== "end";
		const position = start$1 ? 1 : -1;
		const date = hasRangeValue(dValue) ? start$1 ? dValue.start : dValue.end : dValue;
...
function createTimePicker(props) {
	...
	function updateParts(newParts) {
		...
		let newValue = null;
		if (isRange.value) {
			const start$1 = isStart.value ? newParts : dateParts.value[0];
			const end$1 = isStart.value ? dateParts.value[1] : newParts;
			newValue = {
				start: start$1,
				end: end$1
			};
